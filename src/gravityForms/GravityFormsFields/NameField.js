...
export default function NameField({ field, fieldErrors }) {
  const { id, formId, type, label, description, cssClass, inputs, isRequired } = field;
        ...
        return (
          <div key={key}>
            <input
              ...
              onChange={handleChange}
            />
            <label htmlFor={`input_${formId}_${id}_${key}`}>{inputLabel}{isRequired && <span className="is-required">(Required)</span>}</label>
          </div>
        );
      }
      )}
