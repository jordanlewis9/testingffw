...
const pageTemplateFile = path.join(process.cwd(), 'src', 'templates', 'post.js')
const componentsFolder = path.join(process.cwd(), 'src', 'components', 'post')
const cacheFolder = path.join(process.cwd(), '.cache', 'post-templates')
const fragmentsFolder = path.join(process.cwd(), 'src', 'fragments')
const fragmentsFile = path.join(fragmentsFolder, 'postComponents.js')
...
module.exports.makeOptions = theOptions => {
...
const createTemporaryPageTemplateFile = (
  ...
  const componentImportString = componentNames
    .map(componentName => {
      return `import ${componentName} from '../../src/components/post/${componentName}'`
    })
    .join('\n')
